...
class ManagementsControllerClients extends JControllerAdmin
{
	...
	public function sticky_publish()
	{
		...
		if (empty($ids)) {
			JError::raiseWarning(500, JText::_('COM_MANAGEMENTS_NO_CLIENTS_SELECTED'));
		} else {
			// Get the model.
			/** @var ManagementsModelClients $model */
			$model = $this->getModel();
			...
			if (!$model->stick($ids, $value)) {
				...
				if ($value == 1) {
					$ntext = 'COM_MANAGEMENTS_N_CLIENTS_STUCK';
				} else {
					$ntext = 'COM_MANAGEMENTS_N_CLIENTS_UNSTUCK';
				}
		...
		$this->setRedirect('index.php?option=com_managements&view=clients');
    }
}
